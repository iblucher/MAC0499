...
Nas observações utilizadas no desenvolvimento deste trabalho...

% similaridade no contexto dos espectros estelares
No caso dos espectros estelares, o estudo da similaridade entre sinais pode ser útil para alinhar regiões de contaminação atmosférica de moléculas específicas em um espectro de ciência com as regiões correspondentes no espectro telúrico, antes de se estimar o espectro original pela técnica da divisão.

% falhas da distancia euclideana como metrica de similaridade
...
% A figura \ref{fig:euclidean-vs-dtw-matching} ilustra como o DTW encontra uma correspondência mais intuitiva do que a correspondência euclideana, juntando regiões das duas sequências com formatos similares mesmo que não estejam perfeitamente alinhadas.

...
static int mp_fdjac2(
        ...
        int *ddebug, double *ddrtol, double *ddatol, cpl_table** prof_out);
static void mp_qrfac(
...
#define mp_call(funct, m, n, x, fvec, dvec, priv, prof) (*(funct))(m,n,x,fvec,dvec,priv,prof)
...
int mpfit(mp_func funct, int m, int npar,
        ...
        mp_result *result, cpl_table** prof_out)
{
    mp_config conf;
    ...
    iflag = mp_call(funct, m, npar, xall, fvec, 0, private_data, prof_out);
    nfev += 1;
    if (iflag < 0) {
        goto CLEANUP;
    }
    ...
    do {
        ...
        do {
            ...
            iflag = mp_call(funct, m, npar, xnew, wa4, 0, private_data, prof_out);
            nfev += 1;
            if (iflag < 0) {
                broken = 1;
                break;
        ...
        int *ddebug, double *ddrtol, double *ddatol, cpl_table** prof_out)
{
    ...
    *
    ...
    if (has_analytical_deriv) {
        iflag = mp_call(funct, m, npar, x, wa, dvec, priv, prof_out);
        if (nfev) *nfev = *nfev + 1;
        if (iflag < 0 ) {
            if (dvec) free(dvec);
            return iflag;
    ...
    if (has_numerical_deriv) for (j=0; j<n; j++) {  /* Loop thru free parms */
        ...
        x[ifree[j]] = temp + h;
        iflag = mp_call(funct, m, npar, x, wa, 0, priv, prof_out);
        if (nfev) *nfev = *nfev + 1;
        if (iflag < 0 ) {
            if (dvec) free(dvec);
            return iflag;
